    ...
    route("/room") {
        authenticate {
            ...
            get {
                ...
                val currentHourMinute = "${now.hour.toString().padStart(2, '0')}${
                ...
                val rooms = collection.find().toList().map { room ->
                    room.apply {
                        val currentLabs = todaysLabs.filter { it.roomIds.contains(this._id) }
                            .filter { it.startTime <= currentHourMinute && currentHourMinute < it.endTime }
                        val upcomingLabs = todaysLabs.filter { it.startTime > currentHourMinute }
                        if (currentLabs.isNotEmpty() || upcomingLabs.isNotEmpty()) {
                            removalChance =
                                currentLabs.maxByOrNull { it.removalChance }?.removalChance
                            nextLab = todaysLabs.filter { it.roomIds.contains(this._id) }
                                .filter { it.startTime > currentHourMinute }
                        ...
                        if (reports.isNotEmpty()) {
                            if (reports.any { it.removalChance != null })
                                removalChance = reports.groupingBy { it.removalChance }.eachCount()
                                    .maxByOrNull { it.value }!!.key!!
                            if (reports.any { it.popularity != null })
                                popularity = reports.groupingBy { it.popularity }.eachCount()
                                    .maxByOrNull { it.value }!!.key!!
                        }
                    }
                }
